...
const authenticateCleverSecret = async (req, res, next) => {
  try {
    const token = req.body.rediectSecret;
    if (token) {
      const {user, error} = await cleverHelper.verifyRediectSecret(token);
      ...
      req.user = {
        cleverRole: user.cleverRole,
        redirectSecret : token,
      }
      next();
    } else {
      return res
...
module.exports = authenticateCleverSecret;
